...
async def async_setup_entry(
    ...
    if p.zone_count > 1 and p.parallel == 1:
        sensor = InputNumberProgram(
            unique_id,
            p.name,
            'inter_zone_delay','sec',CONST_DELAY_OFFSET,-CONST_DELAY_OFFSET,1)
    ...
    if p.start_type == 'sunrise':
        sensor = InputNumberProgram(
            unique_id,
            p.name,
            'sunrise_offset','min',CONST_SUN_OFFSET,-CONST_SUN_OFFSET,30)
        sensors.append (sensor)
        ...
        sensor = InputNumberProgram(
            unique_id,
            p.name,
            'sunset_offset','min',CONST_SUN_OFFSET,-CONST_SUN_OFFSET,30)
        sensors.append (sensor)
    ...
    for i,zone in enumerate(zones):
        ...
        sensor = Water(unique_id,p.name, zone.name, zone.flow_sensor,p.water_max,p.water_step)
...
class InputNumberProgram(RestoreNumber):
    ...
    def __init__(self, unique_id,pname, translation_key,unit_of_measure,max,min,step ):
